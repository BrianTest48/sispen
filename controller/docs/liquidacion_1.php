...
$m_total = $anios_sueldo + $meses_sueldo + $adelanto + $vacaciones + $gratificaciones + $reintegro + $incentivo + $bonif + $bonif_extra + $bonif_grac + $bonif_meta  + $bonif_fest;
...
$table->addRow();
$table->addCell(4500)->addText("INICIO DE LABORES",'font-md');
$table->addCell(4500)->addText($fecha_inicio,'font-md');
$table->addRow();
$table->addCell(4500)->addText("CESE DE LABORES",'font-md');
...
$section->addTextBreak(1);
$section->addText('He recibido de la empresa '.$nombre_emp.', la suma de '.$moneda." ".$montototal.'('.$monto_letra.'), por el concepto de mi indemnización por mi tiempo de servicios, conforme a ley.','font-sm','text-just');
$section->addText('Declaro que, con este pago, no tengo nada que reclamar a esta empresa, habiendo también recibido mi Certificado de Servicios.','font-sm','text-just');
$section->addText('Por lo tanto, firmo el presente documento dando fé de lo recibido, estando totalmente CONFORME.','font-sm','text-just');
...
exportarWord::write($phpWord, $directorio . $nombre_carpeta, $nombre_afi.'-LQ1-'.$ruc, $writers);
sleep(1);
