...
$(document).ready(function(){ 
    tabla=$('#lista_data').dataTable({
        ...
        "ajax":{
            url: '../../controller/listacontrolador.php?op=listar',
            type : "get",
            dataType : "json",
            error: function(e){
                console.log(e);
...
function guardaryeditar(e){
    ...
    $.ajax({
        url: "../../controller/listacontrolador.php?op=guardaryeditar",
        type: "POST",
        data: formData,
        contentType: false,
        processData: false,
...
function editar(lista_id){
    ...
    let tipo ;
   $.post("../../controller/listacontrolador.php?op=mostrar_id",{ lista_id : lista_id},function(datos){
        if(datos != ""){
            //console.log(datos);
            datos = JSON.parse(datos);
            let cnt = datos.cantidad;
            $("#lista").val(datos.id);
            $("#af_id").val(datos.id_afiliado);
            $("#tipo").val(datos.tipo);
            $("#txtdate").val(datos.fech_nac);
            $("#nombre_af").val(datos.nombres + ' '+ datos.ap_pa);
            $("#documento").val(datos.num_doc);
   ...
   // $('#modalmantenimientolistas').modal('show');
}
...
function eliminar(lista_id){
    swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.post("../../controller/listacontrolador.php?op=eliminar",{lista_id:lista_id},function (data) {
